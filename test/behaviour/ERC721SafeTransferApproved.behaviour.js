...
export default function shouldBehaviourSafeTransferApproved(owners, newOwner, approved, value, data) {
  ...
  it("古い順に渡す", async function() {
    ...
    for (i = 0; i < 12; i++) {
      ...
      if (!data) {
        await safeTransferFrom.sendTransaction(owner, newOwner, tokenId, {from: approved, value: value, gas: TEST_GAS})
      } else {
        await safeTransferFrom.sendTransaction(owner, newOwner, tokenId, data, {from: approved, value: value, gas: TEST_GAS})
      }
    }
